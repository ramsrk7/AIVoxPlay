...
class OrpheusAudioProcessor:
    ...
    @staticmethod
    def convert_to_audio(multiframe: list[int], count: int):
        """
        Fast path: no Python-side loops, no torch.cat in a hotspot.
        Accepts a flat list of token ints whose length is a multiple of 7.
        Returns pcm16-le bytes or None when input is invalid.
        """
        if len(multiframe) < 7 or len(multiframe) % 7:
            return None

        # →  (N, 7)  numpy array   (cheap – just a view unless copy needed)
        arr = np.asarray(multiframe, dtype=np.int32).reshape(-1, 7)

        # quick range check before any torch work
        if arr.min() < 0 or arr.max() > 4096:
            return None

        # gather the codebooks in one go; shape (1, total_codes)
        c0 = torch.from_numpy(arr[:, 0]).to(snac_device).unsqueeze(0)
        c1 = torch.from_numpy(arr[:, [1, 4]].ravel()).to(snac_device).unsqueeze(0)
        c2 = torch.from_numpy(arr[:, [2, 3, 5, 6]].ravel()).to(snac_device).unsqueeze(0)
        codes = [c0, c1, c2]

        with torch.inference_mode():
            audio_hat = model.decode(codes)

        # slice + convert on device → fewer host transfers
        audio_int16 = (
            audio_hat[:, :, 2048:4096]          # (1 ,1 ,n)
            .mul(32767.0)                       #  <-- out-of-place
            .clamp(-32768, 32767)
